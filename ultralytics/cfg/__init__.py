...
def entrypoint(debug=""):
    ...
    if not args:  # no arguments passed
        ...
        return
    if "distill-train" in args:
        if "obb" not in args:
            raise ValueError("distill-train is currently defined only for the OBB task")
        from ultralytics.models.yolo.obb.distill_train import DistillOBBTrainer

        distill_args = [arg for arg in merge_equals_args(args) if arg not in {"obb", "distill-train"}]
        overrides = {}
        for argument in distill_args:
            if "=" not in argument:
                raise SyntaxError(f"PRCD arguments must use key=value syntax, got {argument!r}")
